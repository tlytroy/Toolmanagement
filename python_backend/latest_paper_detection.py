...
def detect_paper_corners_latest(image_array: np.ndarray) -> List[Dict[str, float]]:
    """
    使用最新的鲁棒算法检测A4纸四角
    """
    try:
        ...
        print(f"最新纸张检测算法执行出错: {e}")
        return []

def warp_paper_latest(image_array: np.ndarray, corners: List[Dict[str, float]]) -> np.ndarray:
    """
    使用最新的透视校正算法，修复图像被压扁的问题
    """
    try:
        # 转换角点格式
        pts = np.array([[p["x"], p["y"]] for p in corners], dtype=np.float32)
        
        # 使用固定的A4尺寸进行透视校正，避免图像被压扁
        # A4标准尺寸：210mm x 297mm，使用4px/mm的分辨率
        width = 210 * 4  # 840 pixels
        height = 297 * 4  # 1188 pixels
...
def test_latest_paper_detection():
    """测试最新的纸张检测"""
    print("测试最新的纸张检测算法...")
    ...
    # 检测纸张角点
    corners = detect_paper_corners_latest(image)
    
    if corners:
        print(f"✓ 成功检测到纸张角点: {corners}")
        
        # 进行透视校正
        warped = warp_paper_latest(image, corners)
        ...
        # 保存结果
        cv2.imwrite('test_results/paper_detection/latest_paper_detection.jpg', result_img)
        cv2.imwrite('test_results/paper_detection/latest_warped_result.jpg', warped)
        print("✓ 最新纸张检测结果已保存到: test_results/paper_detection/latest_paper_detection.jpg")
        print("✓ 透视校正结果已保存到: test_results/paper_detection/latest_warped_result.jpg")
